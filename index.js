...
const atividade = {
  nome: "Almoço",
  data: new Date("2024-07-08 10:00"), // Serve para criar uma data e segue o modelo aa-mm-dd
  finalizada: false,
}
...
const criarItemDeAtividade = (atividade) => {
  ...
  return `
    <div class="card-bg">
            ...
            <time class="full" datetime="">${formatar.dia.semana.longo}, dia ${formatar.dia.numerico} de ${formatar.mes} às ${formatar.hora}h</time>
    </div>
    `
}
...
const salvarAtividade = (event) => {
  ...
  const novaAtividade = {
    nome,
    data, // Serve para criar uma data e segue o modelo aa-mm-dd
    finalizada: false
  }
...
const criarHorasSelecao = () => {
  ...
  for(let i = 6; i < 23; i++) {
    ...
    horasDisponives += `<option value="${hora}:00">${hora}:00</option>`
    horasDisponives += `<option value="${hora}:30">${hora}:30</option>`
  }

  document.querySelector('select[name="hora"').innerHTML = horasDisponives
}
criarHorasSelecao()
